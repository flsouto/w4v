...
pub fn resize(input_wav: &[u8], new_duration: f32) -> Result<Vec<u8>, String> {
    if new_duration <= 0.0 {
        return Err("New duration must be positive.".to_string());
    }

    let current_duration = len(input_wav)?;
    if current_duration == 0.0 {
        // Cannot determine speed factor if original duration is 0
        return Ok(input_wav.to_vec());
    }
...
pub fn resize_js(input_wav: &[u8], new_duration: f32) -> Result<js_sys::Uint8Array, JsValue> {
    match resize(input_wav, new_duration) {
...
mod tests {
    ...
    fn test_resize_effect() {
        ...
        let output_wav_shorter = resize(&input_wav, shorter_duration).expect("Resize function failed for shorter duration");
        let processed_shorter_duration = len(&output_wav_shorter).expect("Failed to get processed shorter duration");
        ...
        let output_wav_longer = resize(&input_wav, longer_duration).expect("Resize function failed for longer duration");
        let processed_longer_duration = len(&output_wav_longer).expect("Failed to get processed longer duration");
